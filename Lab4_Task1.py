...
# Updated for Task: Landmark Localization
# Path: /Users/varennyamjoshi/FAIRIS-Lite/WebotsSim/controllers/Lab4_Task1.py
...
while robot.experiment_supervisor.step(robot.timestep) != -1:
    ...
    for obj in objects:
            ...
            align = position_on_image_list[0]
                #if align - 360 <= 10
            # Check if the colors match the desired colors
            if align - 360 <= 10:
                goal = obj
                break

    for obj_id, obj_info in objects.items():
        if obj_id in landmarks:
            distance = estimate_distance_to_landmark(landmarks[obj_id])
            distances_to_landmarks[obj_id] = distance

    if len(distances_to_landmarks) >= 3:
      
       position = estimate_position_from_landmarks(distances_to_landmarks)
       orientation = get_robot_orientation()
        
       print(f"Position: {position}")
       # Compute current pose based on landmarks
    compute_pose_from_landmarks()
